...
public class CommandEC extends CommandBase {
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException { 
        EnumCommandArg arg;
        if ((arg = EnumCommandArg.get(args)) == null)         
            throw new WrongUsageException(this.getUsage(sender));   
        EntityPlayerMP player = getCommandSenderAsPlayer(sender);  
        switch(arg) {
        case HELP:
            EnumChatMessage.COMMAND_EC_HELP.sendMessage(player);
            break;
        case LIST_ALL:
            if (!this.validAction(player, false, false, false, false)) break;
            EnumChatMessage.COMMAND_EC_LIST_ALL.sendMessage(player, arg.getProcessingArgument(args));
            break;
        case FILE_ALL:
            ...
            EnumChatMessage.COMMAND_EC_FILE_ALL.sendMessage(player);
            break;
        case LIST_UNKNOWN:
            if (!this.validAction(player, false, false, true, false)) break;
            EnumChatMessage.COMMAND_EC_LIST_UNKNOWN.sendMessage(player, arg.getProcessingArgument(args));
            break;
        case FILE_UNKNOWN:
            ...
            EnumChatMessage.COMMAND_EC_FILE_UNKNOWN.sendMessage(player);
            break;
        case CLEAR:
            if (!this.validAction(player, true, true, false, true)) break;
            ConfigLoader.clear();
            EnumChatMessage.COMMAND_EC_CLEAR.sendMessage(player);
            break;
        case RELOAD:
            if (!this.validAction(player, true, true, false, true)) break;
            ...
            ConfigLoader.load();
            this.wrap();
            EnumChatMessage.COMMAND_EC_RELOAD.sendMessage(player);
            break;
        case BACKUP:
            if (!this.validAction(player, true, true, false, true)) break;
            ConfigLoader.backup();
            EnumChatMessage.COMMAND_EC_BACKUP.sendMessage(player);
            break;
        case UPDATE:
            if (!this.validAction(player, true, true, true, true)) break;
            ConfigLoader.save();
            EnumChatMessage.COMMAND_EC_UPDATE.sendMessage(player);
            break;
        case INFO:
            if (!this.validAction(player, false, false, false, false)) break;
            EnumChatMessage.COMMAND_EC_INFO.sendMessage(player, arg.getProcessingArgument(args));
            break;
        case EVAL:
            if (!this.validAction(player, false, false, false, false)) break;
    ...
    private boolean validAction(EntityPlayer player, boolean checkExternalConfig, boolean checkDebugMode, boolean checkUnknown, boolean checkDedicatedServer) {
        if (checkExternalConfig && !EnumConfigSettings.EXTERNAL_CONFIG.isEnabled()) {
            EnumChatMessage.COMMAND_EC_ERR_EXTERNAL_CONFIG_DISABLED.sendMessage(player);
            return false;
        } else if (checkDebugMode && !EnumConfigSettings.DEBUG_MODE.isEnabled()) {
            EnumChatMessage.COMMAND_EC_ERR_DEBUG_MODE_DISABLED.sendMessage(player);
            return false;
        } else if (checkUnknown && EnchantmentWrapper.UNKNOWN.isEmpty()) {
            EnumChatMessage.COMMAND_EC_ERR_NO_UNKNOWN_ENCHANTMENTS.sendMessage(player);
            return false;
        } else if (checkDedicatedServer && ConfigLoader.runningAtDedicatedServer) {
            EnumChatMessage.COMMAND_EC_ERR_RUNNING_AT_DEDICATED_SERVER.sendMessage(player);
            return false;
        }
        return true;
    }
